...
FExeXModule* FExeXModule::instance = nullptr;

TX_CONTEXTHANDLE FExeXModule::hContext = TX_EMPTY_HANDLE;


TX_TICKET FExeXModule::hConnectionStateChangedTicket = TX_INVALID_TICKET;

TX_TICKET FExeXModule::hEventHandlerTicket = TX_INVALID_TICKET;
...
void OnGazeDataEvent(TX_HANDLE hGazeDataBehavior)
{
	...
	if (txGetGazePointDataEventParams(hGazeDataBehavior, &eventParams) == TX_RESULT_OK) {
		...
		FExeXModule::Get()->SetEyeCoords((int)eventParams.X, (int)eventParams.Y);
	}
	else {
...
void FExeXModule::Shutdown()
{
	...
	if (!success) {
		qInfo() << "\nEyeX could not be shut down cleanly.Did you remember to release all handles ? \n";
	}
}

void FExeXModule::GetEyeCoords(int& OutX, int& OutY)
{
	OutX = EyeX;
	OutY = EyeY;
}

void FExeXModule::SetEyeCoords(int InX, int InY)
{
	EyeX = InX;
	EyeY = InY;
}
